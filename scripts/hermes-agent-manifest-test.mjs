...
try {
  ...
  assert.ok(toolNames.includes('garmin_agent_manifest'), 'Hermes-ready agent manifest tool should be registered.');
  assert.ok(toolNames.includes('garmin_auth_instructions'), 'Agents need safe auth guidance.');

  const resources = await client.listResources();
  const resourceUris = resources.resources.map((resource) => resource.uri);
  assert.ok(resourceUris.includes('garmin://agent-manifest'), 'Agent manifest resource should be registered.');

  const manifestResult = await client.callTool({
    name: 'garmin_agent_manifest',
  ...
  assert.equal(manifest.auth.provider, 'Garmin Connect');
  ...
  assert.ok(manifest.agent_rules.some((rule) => /passwords or tokens/i.test(rule)));

  const manifestResource = await client.readResource({ uri: 'garmin://agent-manifest' });
  const text = manifestResource.contents[0]?.text ?? '';
  assert.match(text, /mcp_garmin_garmin_connection_status/);
  assert.match(text, /\/reload-mcp/);
  assert.match(text, /Unofficial personal token mode/);
  ...
  assert.ok(setupPayload.hermes_skill_path.endsWith('.hermes/skills/garmin-mcp/SKILL.md'));
  ...
  const mergeDir = mkdtempSync(join(tmpdir(), 'Garmin MCP-hermes-merge-'));
  ...
  assert.match(mergedConfig, /garmin:/);
