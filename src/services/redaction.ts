const SENSITIVE_KEY_PATTERN = /^(access[_-]?token|refresh[_-]?token|id[_-]?token|di[_-]?token|di[_-]?refresh[_-]?token|jwt[_-]?web|csrf[_-]?token|client[_-]?secret|authorization|password|api[_-]?key)$/i;

const SECRET_VALUE_PATTERNS: RegExp[] = [
  /Bearer\s+[A-Za-z0-9._~+/=-]+/gi,
  /(access_token["'=:\s]+)[A-Za-z0-9._~+/=-]+/gi,
  /(refresh_token["'=:\s]+)[A-Za-z0-9._~+/=-]+/gi,
  /(di_token["'=:\s]+)[A-Za-z0-9._~+/=-]+/gi,
  /(di_refresh_token["'=:\s]+)[A-Za-z0-9._~+/=-]+/gi,
  /(jwt_web["'=:\s]+)[A-Za-z0-9._~+/=-]+/gi,
